...
- `Journey`
  ...
  - `kind`: `travel | commute`
  - `entries`: 时间线条目数组
  - `trackLocations`: 持续定位期间记录的 GPS 点位数组（`TimelineLocation[]`）
- `TimelineEntry`
...
- 模板读取失败回退默认配置。
- Journey 读取失败回退空数组。
- `trackLocations` 缺失时规范化为空数组（兼容旧版本数据）。
...
### 6.4 持续定位（Continuous GPS Tracking）

- 开关 Toggle 位于当前旅程卡片标题下方，标签为"持续定位"。
- 开启时：调用 `Location.watchPositionAsync`，精度 `Highest`，每 5 秒或移动 5 米记录一个 GPS 点位。
- 点位缓冲在 `trackedLocations` ref 中，每次组件渲染时批量同步到当前旅程的 `trackLocations` 数组并持久化。
- 关闭时：停止 `watchPositionAsync`，将缓冲中的点位全部写入旅程。
- 新建旅程自动初始化 `trackLocations: []`。
- 数据迁移：`loadJourneys` 读取旧版本数据时，将缺失的 `trackLocations` 字段规范化为空数组。
