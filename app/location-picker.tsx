...
export default function LocationPickerScreen() {
  ...
  const refreshNearbyPlaces = useCallback(
    async (target: AMapLatLng) => {
      if (!amapWebKey) {
        setNearbyHint('未配置 EXPO_PUBLIC_AMAP_WEB_KEY，无法加载附近地点。');
        setNearbyPlaces([]);
        return;
      }
      ...
      try {
        ...
        if (list.length === 0) {
          setNearbyHint('附近地点为空，请在地图上重新选择位置。');
        }
      } catch {
        setNearbyPlaces([]);
        setNearbyHint('附近地点加载失败，请检查网络或高德 Web Key。');
      } finally {
  ...
  const locateCurrent = useCallback(async () => {
    ...
    try {
      ...
      if (screenActiveRef.current && !hasPosition) {
        Alert.alert('定位失败', '无法获取当前位置，请手动点选地图。');
      }
    } finally {
      if (screenActiveRef.current) {
  ...
  useEffect(() => {
    if (Platform.OS !== 'android') {
      setSdkReady(false);
      setSdkError('当前仅支持 Android 地图选点。');
      return;
    }
  ...
  async function confirmLocation() {
    if (!selected || !selectedWgs) {
      Alert.alert('请先选点', '请点击地图或下方地点列表后再确认。');
      return;
    }
    setSaving(true);
    try {
    ...
    if (!mapVisible) {
      return (
        <View style={styles.mapFallback}>
          <Text style={[styles.mapFallbackText, theme.muted]}>正在保存并返回...</Text>
        </View>
      );
    }
    ...
    <View style={[styles.page, theme.page, { paddingTop: insets.top + 6 }]}>
      ...
      <View style={[styles.mapSection, theme.card]}>
        <View style={styles.mapHeader}>
          <Text style={[styles.sectionTitle, theme.title]}>地图选点</Text>
          <Pressable style={styles.locateButton} onPress={() => void locateCurrent()} disabled={loadingLocation}>
            <MaterialIcons name="my-location" size={16} color="#ffffff" />
            <Text style={styles.locateButtonText}>{loadingLocation ? '定位中...' : '当前位置'}</Text>
          </Pressable>
        </View>
      ...
      <View style={[styles.listSection, theme.card]}>
        ...
        <Pressable style={styles.confirmButton} onPress={() => void confirmLocation()} disabled={saving}>
          <Text style={styles.confirmButtonText}>{saving ? '保存中...' : '确认使用该位置'}</Text>
        </Pressable>
      </View>
    </View>
  );
